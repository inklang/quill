...
impl Command for Doctor {
    async fn execute(&self, ctx: &Context) -> Result<()> {
        ...
        // Check compiler available (printing_press is built into quill binary)
        println!("Checking compiler...");
        match printing_press::compile("fn main() {}", "check") {
            Ok(_) => println!("  [OK] Compiler (printing_press) is available"),
            Err(e) => {
                println!("  [WARN] Compiler error: {}", e.display());
                all_ok = false;
            }
        }
